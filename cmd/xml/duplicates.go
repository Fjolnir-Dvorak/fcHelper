...
func init() {
	XmlCmd.AddCommand(duplicateCmd)
	duplicateCmd.Flags().StringVarP(&compOld, "base", "b", "", "Old xml version.")
	duplicateCmd.Flags().StringVarP(&compNew, "new", "n", "", "New xml version.")
	duplicateCmd.Flags().StringVarP(&compOut, "out", "o", out, "Output directory")
...
func doDuplicate(cmd *cobra.Command, args []string) {
	// Load both files
